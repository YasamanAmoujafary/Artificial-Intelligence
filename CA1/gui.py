...
def solver_bfs(game_map):
    ...
    def bfs():
        while queue:
            ...
            game.set_player_position(curr_state[0])
            game.set_box_positions(list(curr_state[1]))
...
def heuristic_2(game):
    ...
    for i, box_loc in enumerate(box_locs):
        ...
        if is_corner(game, i) and box_loc != goal_locs[i]:
            manhattan_distance += 1000
    return manhattan_distance

def solver_astar(game_map, heuristic_func=heuristic_1, weight=1):
    game = Game(game_map)
    visited = set()
    queue = PriorityQueue()
    parent = dict()
    ...
    while not queue.empty():
        current_f, curr_state = queue.get()
        game.set_player_position(curr_state[0])
        game.set_box_positions(list(curr_state[1]))
        ...
        for direction in ['L', 'R', 'U', 'D']:
            ...
            if game.apply_move(direction):
                new_state = (game.get_player_position(), tuple(game.get_box_locations()))
                move_cost = 1
                new_cost = cost[curr_state] + move_cost

                if new_state not in visited :
                    visited.add(new_state)
                    cost[new_state] = new_cost
                    parent[new_state] = (curr_state, direction)
                    priority = new_cost + weight * heuristic_func(game)
